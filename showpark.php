...
if($result->num_rows==0)
{
	...
	if($result->num_rows>0)
	{
		...
		while($row=$result->fetch_assoc())
		{
			...
			if($st=="available")
			{
			echo "<tr><td>Parking Slots".$sl."</td></tr> "<tr><td>Parking Slots".$sl."</td></tr>"<tr><td>Parking Slots".$sl."</td></tr>";
			}
		}
		$_SESSION['page']=$check;
		...
		echo "Entry  Time::<input type='text' required='required' name='entry' placeholder='Entry Time'>";
		echo "Exit Time::<input type='text' required='required' name='exit' placeholder='Exit Time'><button>Book Now</button></form></table>";
